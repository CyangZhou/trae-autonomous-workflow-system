...
class LongTermMemory:
    ...
    def _extract_entities(self, interaction: Dict[str, Any]) -> Dict[str, Dict]:
        """从交互中提取实体"""
        entities = {}
        ...
        for kw in tech_keywords:
            entities[kw] = {
                "type": "technology",
                "usage_count": entities.get(kw, {}).get("usage_count", 0) + 1
            }
        
        return entities

    def _extract_relations(self, interaction: Dict[str, Any]) -> Dict[str, List[Tuple[str, str]]]:
        """从交互中提取关系"""
        relations = {}
        action = interaction.get('action', '').lower()
    ...
    def evolve(self, new_interaction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 动态更新实体状态
        entities = self._extract_entities(new_interaction)
        for entity_id, entity_data in entities.items():
            self.world_model.update_entity_state(entity_id, entity_data)

        # 构建知识图谱关系
        relations = self._extract_relations(new_interaction)
        for rel_type, pairs in relations.items():
            if rel_type == "uses":
                for source, target in pairs:
                    self.world_model.add_relation(source, target, rel_type)
